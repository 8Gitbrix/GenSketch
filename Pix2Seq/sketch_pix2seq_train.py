...
def load_dataset(data_dir, model_params, inference_mode=False):
    ...
    for dataset in datasets:
        ...
        if train_strokes is None:
            train_strokes = data['train'][:1000]  # [N (#sketches),], each with [S (#points), 3]
            valid_strokes = data['valid'][:200]
            test_strokes = data['test'][:200]
        else:
...
def save_model(sess, model_save_path, global_step):
    ...
    newpath = "/content/gdrive/My Drive/ML_Project/snapshot"
    if os.path.exists(newpath):
        shutil.rmtree(newpath)
    shutil.copytree("/content/outputs" , "/content/gdrive/My Drive/ML_Project/snapshot")
...
def train(sess, model, eval_model, train_set, valid_set, test_set):
    ...
    start = time.time()
    print(hps.num_steps)
    for num in range(hps.num_steps):
        print(num)
